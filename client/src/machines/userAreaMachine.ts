import { assign, createMachine } from "xstate";

export const userAreaMachine = createMachine({
  /** @xstate-layout N4IgpgJg5mDOIC5RgHZgLYE8B0BjAFgIawCWKUAxAIIAqNVAwgNIDaADALqKgAOA9qQAuJPim4gAHogBsbACzZpADgBMKpQGZpclXICs6gDQhMiPQEYAnNhWW5Viyr1tLLgL5vjqDDgLEylADKNACqAHLsXEgg--QBXFBoCDPToMADyADVOk9UzFDSzMwgCspsPJzJkeUWjnIy4XsIU5KUdPzeYW9C2mu3O5AKH6A0HCKGwJHzBtY-GPUeu6P--7cMx4HsaUcG8f69QqLFonWdcdQ1NmwgIBps7hTKKjyj0b9ticTc+ryVnsHLUoK3KOy8ydDwPCAA */
  id: "enemy",
  initial: "chasing",
  context: {
    initialHP: 100,
    currentHP: 100,
    playerIsReachable: true,
  },
  states: {
    chasing: {
      on: {
        ATTACK: "attacking",
        STUN: "stunned",
        TAKE_DAMAGE: "takeDamage",
        RUN_AWAY: "runaway",
      },
    },
    attacking: {
      on: {
        FINISH_ATTACK: [
          {
            target: "attacking",
            cond: (context) => !context.playerIsReachable,
          },
          {
            target: "chasing",
            cond: (context) => context.playerIsReachable,
          },
        ],
      },
      after: {
        500: "chasing",
      },
    },
    stunned: {
      on: {
        RECOVER: "chasing",
      },
      after: {
        1000: "chasing",
      },
    },
    takeDamage: {
      entry: assign({ currentHP: (context) => context.currentHP - 20 }),
      on: {
        STUN: "stunned",
        DIE: "dead",
        CHASE: "chasing",
      },
      after: {
        500: "chasing",
      },
    },
    runaway: {
      on: {
        STOP_RUN_AWAY: "chasing",
      },
    },
    dead: {
      type: "final",
    },
  },
  on: {
    PLAYER_REACHABLE_CHANGE: {
      actions: assign((context, event) => ({
        ...context,
        playerIsReachable: event.reachable,
      })),
    },
  },
  // on: {
  //   PLAYER_REACHABLE_CHANGE: {
  //     actions: assign({
  //       playerIsReachable: (_, event) => {
  //         alert(JSON.stringify(_)); // {"initialHP":100,"currentHP":100,"playerIsReachable":false}
  //         alert(JSON.stringify(event)); //{"type":"PLAYER_REACHABLE_CHANGE","reachable":true}

  //         return event.reachable;
  //       },
  //     }),
  //   },
  // },
  // @ts-ignore
  actions: {
    reduceHP: assign({
      currentHP: (context) => context.currentHP - 1,
    }),
  },
});
